...
item=$((${#options[*]}-1))
eval ${options[$item]}=$item
description[$item]="Name of your materials key CSV file.  Data be in format:  Name, Index, Conductivity, HeatProduction.  First line a header."
takesargument[${#takesargument[*]}]=$item
defaultvalue[$item]=$NODEFAULT
testfntext=$(cat <<EOF                                       # define a test function
   function ${flag}Test () {
...
while IFS=$'\n' read -r LINE || [[ -n "$LINE" ]]; do
   ...
   if [ $returnVal -eq 0 ]; then
      ...
      matConduct[${#matConduct[*]}]=$var3 
      matHeat[${#matHeat[*]}]=$var4 
   fi 
done <  ${keyFilenameArg}
