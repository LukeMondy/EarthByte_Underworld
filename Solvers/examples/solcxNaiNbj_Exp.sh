...
#
#   Solve using BSSCR with Viscous Penalty Method from Experimental
#
# Options:
...
for PEN in 0.0 10.0 100.0 10000.0
do
...
$UWEXEC $UWPATH/Underworld/SysTest/PerformanceTests/testVelicSolCx.xml \
    $UWPATH/Experimental/InputFiles/StokesPIC+AugmentedLagrangianBSSCR.xml \
    ...
    $MGOP \
                -NN $OUT -dump_matvec -matsuffix "n$count" \
                --particlesPerCell=$PP \
  		...
  		--saveDataEvery=1 --checkpointEvery=2 --checkpointWritePath="./$OUT/Checkpoints" --checkpointAppendStep=1 \
                --components.constitutiveMatrix.incompressibility_Penalty=$PEN \
                ...
                -XscrPCKSP_ksp_view \
  		-ksp_k2_type NULL --penaltyNumber=0 \
  		-Q22_pc_type $UW \
