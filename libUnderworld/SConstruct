...
# For each of the hg repos detected, identify each one and
# extract repo info (revision, etc.) to be used for our defines later on.
import re
from string import upper
# Check if we can use mercurial api libraries.
try:
    from mercurial import commands, error, hg, ui, util, node
# If we can't, create default info.
except ImportError:
    ...
    # Create local user interface for our repo.
    localui = ui.ui()
    for r in repos:
        try:
            # Retrieve hg repo instance.
            repo = hg.repository(localui, r)
        except RepoError:
            # If repo is not found/valid, create default info.
            ...
            # Get repo actual user interface.
            localui = repo.ui
            try:
                ctx = repo[None]
            except TypeError:
            ...
            path = re.sub("default\s*=\s*","", path)
            path = path.strip()
            localui.pushbuffer()
            commands.branch(localui, repo)
            branch = localui.popbuffer()
            branch = branch.strip()
            from string import upper
            if r == '.':
                r = 'ROOT'
            repo_name = upper(re.sub('\W', '', r))
            # Pass repo path, revision info as defines to compilation.
            env.AppendUnique(IDDEFINES=[(repo_name, env["ESCAPE"]('"' + revision + '"'), \
                             env["ESCAPE"]('"' + path + '"'),  env["ESCAPE"]('"' + branch + '"'))])
