...
import StGermain_Tools as _StGermain_Tools
import StGermain
import StgDomain
import StgFEM
import Underworld
...
def GetCurrentDictionary():
    ...
    if _data == None:
        ...
        return
    xmlString = _StGermain_Tools.StgDictAsXMLString( _data )
...
## We might want to move these basic functions to uwpytools.utils so we are not importing so much
...
def SetDictionary( pyDict, tag=_os.path.realpath(_sys.argv[0]) ):
    ...
    _StGermain_Tools.StgSetDictFromXMLString(_data,xmlString, tag)
...
def AddToDictionary( pyDict, tag=_os.path.realpath(_sys.argv[0]) ):
    ...
    root = _dictToUWElementTree(pyDict)
    xmlString = _ET.tostring(root, encoding = 'utf-8', method = 'xml')
    _StGermain_Tools.StgAddToDictFromXMLString(_data, xmlString, tag)
...
def Init():
    ...
    _data  = _StGermain_Tools.StgInit( [_sys.argv[0]] )
...
def InitWithArgs( args ):
    ...
    _data  = _StGermain_Tools.StgInit( args )
...
def Construct( PushPythonDictionary=True ):
    ...
    _StGermain_Tools.StgConstruct(_data)
    _isConstructed = True
...
def BuildAndInitialise():
    ...
    if _isBuilt == True:
        ...
        return
    _StGermain_Tools.StgBuildAndInitialise(_data)
    _isBuilt = True
...
def RunMainLoop():
    ...
    if _isBuilt == False:
        ...
        return
    _StGermain_Tools.StgMainLoop(_data)
...
def Finalise():
    ...
    if _data == None:
        print "StGermain has not been initialised yet. You need to run one of the Init functions."
        return
    _StGermain_Tools.StgFinalise( _data )
    _resetAll()
...
def SetOutputPath( outputPath ):
    ...
    if _data == None:
        print "StGermain has not been initialised yet. You need to run one of the Init functions."
        return
    _StGermain_Tools.StgSetOutputPath( _data, outputPath )
'''
...
### Global Variables of the uwpytools module should go here. 
### If they are _variableName they remain private (but can be accessed by importing _uwpytools)
